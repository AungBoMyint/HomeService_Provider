...
Future<void> saveUserData(UserData data) async {
  if (data.status == 1) {
    ...
    await userService
        .getUser(email: data.email.validate().toLowerCase())
        .then((value) async {
...
Future<UserInfoResponse> getUserDetail(int id) async {
  return UserInfoResponse.fromJson(await handleResponse(
...
Future<DashboardResponse> providerDashboard() async {
  DashboardResponse data = DashboardResponse.fromJson(await handleResponse(
      await buildHttpResponse('provider-dashboard', method: HttpMethod.GET)));

  if (data.appDownload != null) {
    setValue(PROVIDER_PLAY_STORE_URL,
        data.appDownload!.provider_playstore_url.validate());
    setValue(PROVIDER_APPSTORE_URL,
        data.appDownload!.provider_appstore_url.validate());
  }
  ...
  setValue(IS_ADVANCE_PAYMENT_ALLOWED, data.isAdvancedPaymentAllowed == '1');
...
Future<HandymanDashBoardResponse> handymanDashboard() async {
  HandymanDashBoardResponse data = HandymanDashBoardResponse.fromJson(
      await handleResponse(await buildHttpResponse('handyman-dashboard',
          method: HttpMethod.GET)));
...
Future<UserListResponse> getHandyman(
    {bool isPagination = false,
    int? page,
    int? providerId,
    String? userTypeHandyman = "handyman"}) async {
  if (isPagination) {
    return UserListResponse.fromJson(await handleResponse(await buildHttpResponse(
        'user-list?user_type=$userTypeHandyman&provider_id=$providerId&per_page=$PER_PAGE_ITEM&page=$page',
        method: HttpMethod.GET)));
  } else {
    return UserListResponse.fromJson(await handleResponse(
        await buildHttpResponse(
            'user-list?user_type=$userTypeHandyman&provider_id=$providerId',
            method: HttpMethod.GET)));
  }
}
...
Future<List<UserData>> getAllHandyman(
  ...
  UserListResponse res = UserListResponse.fromJson(
    await handleResponse(await buildHttpResponse(
        'user-list?user_type=handyman&provider_id=${appStore.userId}&per_page=$PER_PAGE_ITEM&page=$page',
        method: HttpMethod.GET)),
  );
...
Future<UserData> deleteHandyman(int id) async {
  return UserData.fromJson(await handleResponse(
      await buildHttpResponse('handyman-delete/$id', method: HttpMethod.POST)));
}
...
Future<CommonResponseModel> deleteService(int id) async {
  return CommonResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('service-delete/$id', method: HttpMethod.POST)));
}
...
Future<SearchListResponse> getServicesList(int page,
  ...
  return SearchListResponse.fromJson(await handleResponse(
      await buildHttpResponse('search-list$req', method: HttpMethod.GET)));
}
...
Future<PlanListResponse> getPricingPlanList() async {
  return PlanListResponse.fromJson(await handleResponse(
      await buildHttpResponse('plan-list', method: HttpMethod.GET)));
}
...
Future<WalletHistoryListResponse> getWalletHistory(int page,
    {var perPage = PER_PAGE_ITEM}) async {
  return WalletHistoryListResponse.fromJson(await handleResponse(
      await buildHttpResponse(
...
Future<TaxListResponse> getTaxList() async {
  return TaxListResponse.fromJson(await handleResponse(
      await buildHttpResponse('tax-list', method: HttpMethod.GET)));
}

Future<NotificationResponse> getNotification(Map request,
    {int? page = 1}) async {
  ...
  notificationResponse.unReadNotificationList =
      res.notificationData!.where((element) => element.readAt == null).toList();
  notificationResponse.readNotificationList =
      res.notificationData!.where((element) => element.readAt != null).toList();
...
Future<TotalEarningResponse> getTotalEarningList(int page,
    {var perPage = PER_PAGE_ITEM}) async {
  return TotalEarningResponse.fromJson(await handleResponse(await buildHttpResponse(
...
Future<PostJobDetailResponse> getPostJobDetail(Map request) async {
  return PostJobDetailResponse.fromJson(await handleResponse(
...
Future<BaseResponseModel> saveBid(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('save-bid',
          request: request, method: HttpMethod.POST)));
}
...
Future<List<BidderData>> getBidList(
  ...
  try {
    var res = MyBidResponse.fromJson(await handleResponse(
        await buildHttpResponse('get-bid-list?orderby=desc',
            method: HttpMethod.GET)));
...
Future<CommonResponseModel> deletePackage(int id) async {
  return CommonResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('package-delete/$id', method: HttpMethod.POST)));
}
...
Future<VerifyTransactionResponse> verifyPayment(
  ...
  return VerifyTransactionResponse.fromJson(await handleResponse(
      await buildHttpResponse(
...
Future<String> sadadLogin(Map request) async {
  var res = await handleResponse(
    await buildHttpResponse('$SADAD_API_URL/api/userbusinesses/login',
        method: HttpMethod.POST, request: request, isSadadPayment: true),
    false,
    true,
  );
